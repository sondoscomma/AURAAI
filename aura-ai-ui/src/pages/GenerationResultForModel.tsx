...
export default function GenerationResultForModel(): JSX.Element {
  ...
  const handleDownloadCurrent = (): void => {
    ...
    link.href = img;
    link.download = `aura-${modelName.replace(/\s+/g, "-").toLowerCase()}-${activeDirection}-${Date.now()}.png`;
  ...
  const handleDownloadAll = (): void => {
    [validFrontImage, validRightImage].forEach((img, i) => {
      ...
      const link = document.createElement("a");
      link.href = img;
      link.download = `aura-${modelName.replace(/\s+/g, "-").toLowerCase()}-${i === 0 ? "front" : "right"}-${Date.now()}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    });
